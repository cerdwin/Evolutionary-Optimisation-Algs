...
def nd_front(population, secondary_population, tertiary_population):
    '''
    pop
    ...
    :param tertiary_population: a list of dictionaries of tuples and corresponding criteria
    :return:
    '''
    ...
    for member in population.keys():
        ...
        for other_member in population.keys():
            ...
            if population[member] < population[other_member] and secondary_population[member] < secondary_population[
                other_member] and tertiary_population[member]<tertiary_population[other_member]:
                tmp = Set_dict[member]
    ...
    return crowding(Frontiers, [population, secondary_population, tertiary_population])
...
def consolidate(function, first_constraint, second_constraint, mutated, selected_population, population_size):
    ...
    second_dict = {}
    third_dict = {}
    for parent_key in selected_population.keys():
        mutated.append(parent_key)
    for offspring in mutated:
        x1 = offspring[0]
        x2 = offspring[1]
        if offspring not in selected_population.keys():
            r0 = eval(function)
            first_dict[offspring] = r0
            r1 = 0
            r2 = 0

            if eval(first_constraint)>0:
                r1 = eval(first_constraint)
                second_dict[offspring] = r1
            else:
                second_dict[offspring] = r1
            if eval(second_constraint)>0:
                r2 = eval(second_constraint)
                third_dict[offspring] = r2
            else:
                third_dict[offspring] = r2

            selected_population[offspring] = [r0, r1+r2]

    return nd_front(first_dict, second_dict, third_dict)
...
def ga(model, generations, mutation_range, x_min, x_max, y_min, y_max, function, first_constraint, second_constraint, initial_population_dict, population_size):
    ...
    for i in range(generations):
        ...
        ### Upravit selection aby vybrala nejlepsi items ###
        ...
        #### ted musis dat dohromady mutated a selected...jakozto populaci rodicu a dat je do dictu, kde value je list s pozici fronty na prvnim miste a crowding distance na druhem
...
if __name__ == '__main__':
    ...
    if model == 'g06': # so usa 100 generacoes, pode nao funcionar quando um constraint esta offseting o outro  = depois o erro e mais ou menos zero
        ...
        first_temp_dict = {}
        second_temp_dict = {}
        first_temp_dict, second_temp_dict, third_temp_dict = population_generator(population_size, x_min, x_max, y_min, y_max, first_constraint, second_constraint, function)

        temp_dict = nd_front(first_temp_dict, second_temp_dict, third_temp_dict)
        ...
        first_temp_dict = {}
        second_temp_dict = {}
        first_temp_dict, second_temp_dict, third_temp_dict = population_generator(population_size, x_min, x_max, y_min,
                                                                                  y_max, first_constraint,

...
def constrained_domination_operator(first, second, first_constraint, second_constraint):
    """
    Alternative to the NSGA-II binary tournament operator, returns True if we are to choose the first solution, False otherwise
    :param first: coordinates corresponding to our first solution
    :param second: coordinates corresponding to second solution
    :param first_constraint: self-explanatory
    :param second_constraint: self-explanatory
    :return: True if first is better, False otherwise
    """
    # first we have to evaluate each solution wrt constraints
    # we
    x1 = first[0]
    x2 = first[1]
    #r0 = eval(function)
    ...
    x2 = second[1]
    #r3 = eval(function)
    r4 = eval(first_constraint)
    r5 = 0
    if second_constraint:
        r5 = eval(second_constraint)

    # 1. if first is feasible and the other one not, we return true
    if (r1<=0 and r2<=0) and (r4>0 or r5>0):
        return True
    # 2. both are infeasible, but i has smaller overall constraint violation
    if (r1>0 or r2>0) and (r4>0 or r5>0):

        if r2+r1 < r4+r5:
            return True
    # 3. if both are feasible but first dominates second:
    if (r1<=0 and r2<=0) and (r4<=0 and r5<=0) and r1<r4 and r2<=r5:
        return True
    return False
...
def selection(population, first_constraint, second_constraint):
    ...
    while len(ret) != to_select:
        tmp = {}
        first = random.choice(list(population.items()))
        for i in range(5):
            second = random.choice(list(population.items()))
            if not constrained_domination_operator(first[0], second[0], first_constraint, second_constraint):
                first = second
        ret[first[0]] = first[1]

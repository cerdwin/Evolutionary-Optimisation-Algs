...
def offsprings(population, first_constraint, second_constraint):
    '''
    offspring generation adjusted for the death penalty...this is to see if we can observe premature fitness stagnation
    :param population:
    ...
    while len(ret) != 100:
        ...
        g1 = eval(first_constraint)
        g2 = eval(second_constraint)
        if g1<=0 and g2<=0:
            print("didn't miss the mark")
            ret.append((x1, x2))
        else:
            counter += 1
...
def mutate(population, x1_mutation_range, x2_mutation_range, first_constraint, second_constraint, function, x1_min, x1_max, x2_min, x2_max):
    ...
    # also mutation here is adjusted to make sure we do not create solutions in breach of constraints
    ret = {}
    counter = 0
